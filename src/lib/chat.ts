import type { Message, ChatState, ToolCall, WeatherResult, MCPResult, ErrorResult, SessionInfo, RiskFlag } from '../../worker/types';
...
class ChatService {
  ...
  async sendMessage(
    message: string,
    model?: string,
    onChunk?: (chunk: string, metadata?: { toolCalls?: ToolCall[] }) => void,
    stream: boolean = true
  ): Promise<ChatResponse> {
    try {
      const shouldStream = stream && !!onChunk;
      const response = await fetch(`${this.baseUrl}/chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message, model, stream: shouldStream }),
      });
      if (!response.ok) {
      ...
      if (shouldStream && response.body) {
        ...
        // After streaming, we need to fetch the final state
  ...
  async getAvailableModels(): Promise<{ success: boolean; data?: { id: string; name: string }[]; error?: string }> {
    try {
